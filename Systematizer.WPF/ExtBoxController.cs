...
class ExtBoxController : BlockController
{
    ...
    {
        UIService.UpdateLinks(VM.Persistent);
    ...
    public override bool ChangeMode(Mode mode, bool saveChanges)
    {
        ...
        if (saveChanges && VM.IsDirty)
        {
            ...
            try
            {
                UIService.SaveBox(VM.Persistent, true);
                if (LinkToPersonIdOnSave != null)
                {
                    UIService.WritePersonLink(new LinkInstruction
                    {
    ...
    public override bool HandleCommand(CommandCenter.Item command)
    {
        ...
        {
            UIService.AbandonBox(VM.Persistent.Box.RowId);
    ...
    void HandleCreateFileCommand()
    {
        ...
        string templatePath = Path.Combine(UIService.GetExeDirectory(), "templates");
    ...
    void HandleDoneCommand()
    {
        ...
        {
            string nextTime = RepeatProjector.AdvanceTime(VM.Persistent.Box.BoxTime, VM.Persistent.Repeats);
            if (nextTime != null)
            {
        ...
        if (doMarkDone)
        {
            ...
            bool hasUndoneChildren = boxId > 0 && UIService.CheckBoxesExist(parentRowId: boxId, filterByNotDone: true);
    ...
    string GetDefaultFolderPath()
    {
        ...
        var boxWithPath = UIService.NavigateToParentBoxWhere(VM.Persistent.Box, b => !string.IsNullOrEmpty(b.RefDir));
        if (boxWithPath != null)
            return boxWithPath.RefDir;
        if (!string.IsNullOrEmpty(VM.RefFile)) return Path.GetDirectoryName(VM.RefDir);
        boxWithPath = UIService.NavigateToParentBoxWhere(VM.Persistent.Box, b => !string.IsNullOrEmpty(b.RefFile));
        if (boxWithPath != null)
