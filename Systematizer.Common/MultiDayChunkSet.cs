...
public class MultiDayChunkSet
{
    ...
    public class DayChunkSet
    {
        ...
        public List<Chunk> Chunks = new(); //first one is catch-all and does not have box ids
    }

    public List<DayChunkSet> Days = new(2);
    ...
    static List<Chunk> CreateDefaults(string date, IEnumerable<CachedBox> scheduledBoxes)
    {
    ...
    public static int GetDefaultChunkIndex(string time)
    {
        ...
        if (int.TryParse(time.AsSpan(8, 2), out int hr))
        {
